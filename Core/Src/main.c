...
int speed; // Speed in decimal
uint8_t speed_BCD; // Speed in BCD
...
int main(void)
{
  ...
  BSP_LCD_GLASS_Clear();
  sprintf(LCD_msg, "%d M/S\r\n", speed);  // format LCD message
  BSP_LCD_GLASS_DisplayString(LCD_msg);  // display speed in m/s to LCD

  // Convert the speed to BCD
  speed_BCD = ((speed / 10) << 4) | (speed % 10);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //Transmit the BCD value
	  HAL_UART_Transmit(&huart1, &speed_BCD, sizeof(speed_BCD), HAL_MAX_DELAY);
	  HAL_Delay(100);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|LD_R_Pin, GPIO_PIN_RESET);
